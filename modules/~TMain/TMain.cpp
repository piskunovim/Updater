...
for (k=0;k<=GM1C; k++) //формируем список для загрузки (учтем файлы/папки исключений на этапе загрузки, потому что может не понадобиться ничего загружать)
	...
	if (download)    {
                ...
                /*                Log->Write("Start downloading Update.7z ...");
                                ...
                                //Form1->idFTP->Get("/Webclient/Update.7z","C:\\DPSUpdate\\Webclient\\Update.7z",TRUE); //качаем архив с обновлением
                                ...
                                bool ExtractOK = false; //флаг, чтобы понять получилось ли у нас разархивировать Update.7z или нет
				AnsiString ArchiveFileName = "C:\\DPSUpdate\\Webclient\\Update.7z"; //место, где лежит скачанный архив

   std::auto_ptr <CSevenZip> SvnZip (new CSevenZip(ArchiveFileName)); //начинаем распаковку
   ...
   if (ExtractOK)
      ...
      if (!DeleteFile(ArchiveFileName))
         ...
         //Раскомментировать если захотим качать пофайлово (не архивом)
                ...
                for(int start=0; start<times; start++)  {
                       try{
                               if (!ExceptionList(FinalPath.c_str())){
                                         ...
                                         bool ExtractOK = false; //флаг, чтобы понять получилось ли у нас разархивировать Update.7z или нет
				         AnsiString ArchiveFileName = FP+".7z"; //место, где лежит скачанный архив
